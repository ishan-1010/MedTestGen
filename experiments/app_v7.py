...
IMPORTED_TESTS_DIR = "data/imported_test_suites"  # NEW: Auto-save imported tests
GENERATED_TESTS_DIR = "data/saved_test_cases"  # NEW: Auto-save generated tests
...
def auto_save_test_cases(test_cases: List[Dict], directory: str, prefix: str = "tests"):
    """Automatically save test cases to files for persistence"""
    ...
    latest_filename = f"{directory}/{prefix}_latest.json"
...
def load_saved_test_cases(directory: str, prefix: str = "tests") -> List[Dict]:
    """Load previously saved test cases from files"""
    os.makedirs(directory, exist_ok=True)
    ...
    try:
        ...
        try:
            files = glob.glob(f"{directory}/{prefix}_*.json")
...
def cleanup_old_saves(directory: str, keep_count: int = 5):
...
def initialize_saved_tests():
    """Load previously saved tests on app startup"""
    saved_generated = load_saved_test_cases(GENERATED_TESTS_DIR, "generated")
    saved_imported = load_saved_test_cases(IMPORTED_TESTS_DIR, "imported")
    return saved_generated, saved_imported
        ...
        if 'last_converted_tests' in st.session_state and st.session_state['last_converted_tests']:
            ...
            with col1:
                if st.button("✅ Add to Test Suite", key="add_imported_to_suite", type="primary"):
                    ...
                    # AUTO-SAVE imported tests
                    saved, filename = auto_save_test_cases(tests_to_add, IMPORTED_TESTS_DIR, "imported")
                    if saved:
                    ...
                    # Also save the updated main suite
                    auto_save_test_cases(st.session_state.generated_tests, GENERATED_TESTS_DIR, "generated")
                    # Clean up old saves
                    cleanup_old_saves(IMPORTED_TESTS_DIR, keep_count=5)
                    cleanup_old_saves(GENERATED_TESTS_DIR, keep_count=5)
...
def main():
    ...
    # Load saved tests on first run
    if 'tests_loaded' not in st.session_state:
        saved_generated, saved_imported = initialize_saved_tests()
        if saved_generated:
            st.session_state.generated_tests = saved_generated
            st.sidebar.success(f"📂 Loaded {len(saved_generated)} saved test cases")
    ...
    with st.sidebar:
        ...
        if os.path.exists(f"{GENERATED_TESTS_DIR}/generated_latest.json"):
            try:
                with open(f"{GENERATED_TESTS_DIR}/generated_latest.json", 'r') as f:
                    saved_tests = json.load(f)
                    st.success(f"✅ {len(saved_tests)} tests auto-saved")
                    file_time = datetime.fromtimestamp(os.path.getmtime(f"{GENERATED_TESTS_DIR}/generated_latest.json"))
